...
var Dependency = (function() {
  ...
  function Dependency(depUrl, config) {
  ...
  Dependency.prototype.cache = function() {
    function cacheSuccessHandler() {
      this.resolved = true;
      var hostConfig = path.join(this.cacheDir, 'parachute.json');
      if (fs.existsSync(hostConfig)) {
        this.config = JSON.parse(fs.readFileSync(hostConfig));
  ...
  Dependency.prototype.deliver = function() {
    ...
    walker.on('file', function(root, fileStats, next) {
      ...
      if (!_.isEmpty(this.config)) {
        ...
        var whitelisted = !this.config.only || _.isArray(this.config.only) && _.any(this.config.only, mm);
