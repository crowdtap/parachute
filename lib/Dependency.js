...
var Dependency = (function() {
  ...
  function Dependency(depUrl, clientConfig) {
    ...
    if (this.url.indexOf('#') > 0) {
      ...
      if (this.isRemote) this.treeish = 'origin/' + this.treeish;
    }
  ...
  Dependency.prototype.checkout = function() {
    function clean()    { return git(['clean', '-d', '-f']); }
    function update()   { return git(['pull']); }
    function checkout() {
      var options = '-f';
      if (this.isRemote) options = options + 't';
      return git(['checkout', options, this.treeish]);
    }

    return clean().then(update).then(checkout.bind(this));
  };
