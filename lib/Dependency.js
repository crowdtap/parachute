  ...
  Dependency.prototype.deliver = function() {
    ...
    walker.on('file', function(root, fileStats, next) {
      ...
      if (_.contains(matchingSrcStr, '**')) {
        // Matching source globbed patterns maintain
        // directory tree of contents matching the glob
        var srcGlobRoot = matchingSrcStr.split('**').shift();
        var destRoot = path.relative(srcGlobRoot, relRoot);
        pathComponents = [ matchingDestStr, destRoot, fileStats.name ];
      } else {
        if (_.last(matchingDestStr) === '/') {
          // Specify a destination directory with a trailing slash
          pathComponents = [ matchingDestStr, fileStats.name ];
        } else if (matchingDestStr) {
          // Specify a destination filename with no trailing slash
          pathComponents = [ matchingDestStr ];
        } else {
          // Strings with no destination specified get copied outright
          pathComponents = [ relRoot, fileStats.name ];
        }
      }

      pathComponents = [ process.cwd(), this.root ].concat(pathComponents);
      dest = path.join.apply(this, _.compact(pathComponents));
      dest = path.resolve(dest);

      mkdirp(path.dirname(dest), function(err) {
        // TODO: Handle mkdirp errors
        if (err) throw err;
        var relDest = path.relative(process.cwd(), dest);
