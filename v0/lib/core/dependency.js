...
var Dependency, EventEmitter, assetVars, copycat, fs, gift, git, path, q, spawn, template, _,
...
Dependency = (function(_super) {
  ...
  function Dependency(src, options) {
    ...
    if (!this.remote) {
      this.src = path.resolve(pathSegments[3], pathSegments[4]);
    }
    this.repo = gift(this.cacheDir);
    this.treeish = pathSegments[5] || 'master';
    if (this.remote && this.treeish !== 'master') {
      this.treeish = "origin/" + this.treeish;
    }
  ...
  Dependency.prototype.cache = function() {
    ...
    git(['clone', this.src.split('#')[0], this.cacheDir]).then(function() {
      return deferred.resolve();
    });
    return deferred.promise;
  };

  Dependency.prototype.copy = function() {
    var deferred,
      _this = this;

    deferred = q.defer();
    this.cache().then(function() {
      return _this.repo.status(function(err, status) {
        if (status.clean) {
          template('action', {
            doing: 'checking out',
            what: _this.treeish
          }).on('data', _this.emit.bind(_this, 'data'));
          return git(['checkout', _this.treeish], {
            cwd: _this.cacheDir
          }).then(function(gitStatus) {
            return _this.copyComponents(function() {
              template('action', {
                doing: 'copying',
                what: _this.src
              }).on('data', _this.emit.bind(_this, 'data'));
  ...
  Dependency.prototype.copyComponents = function(cb) {
    ...
    if (components != null ? components.length : void 0) {
      ...
      return (copyNextComponent = function() {
        ...
        return copycat.copy(component.src, _this.subVariables(component.dest), _this.ncpOptions).then(next);
      })();
    }
  };
